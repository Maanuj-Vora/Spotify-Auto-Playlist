    ...
    if usernames:
        for username in usernames:
            ...
            if not user_validation["valid"]:
                logger.error(
                    f"Invalid user ID '{username}': {user_validation['error']}"
                )
                continue

            if not user_validation["accessible"]:
                logger.warning(
                    f"User '{username}' exists but is not accessible: {user_validation['error']}"
                )
                continue
            ...
            if not playlist_access["accessible"]:
                logger.warning(
                    f"Cannot access playlists for user '{username}': {playlist_access['error']}"
                )
                continue

            logger.info(
                f"User '{username}' validated successfully. Found {playlist_access['playlist_count']} total playlists."
            )
            ...
            try:
                ...
                if user_playlists:
                    ...
                    logger.info(
                        f"Successfully fetched {len(user_playlists)} playlists for user '{username}'"
                    )
    ...
    if playlists_to_track:
        for playlist_id in playlists_to_track:
            ...
            if not playlist_validation["valid"]:
                logger.error(
                    f"Invalid playlist ID '{playlist_id}': {playlist_validation['error']}"
                )
                continue

            if not playlist_validation["accessible"]:
                logger.warning(
                    f"Playlist '{playlist_id}' exists but is not accessible: {playlist_validation['error']}"
                )
                continue

            playlist_info = playlist_validation["playlist_info"]
            logger.info(
                f"Playlist '{playlist_id}' validated successfully: '{playlist_info['name']}' by {playlist_info['owner']['id']}"
            )
            ...
            try:
                ...
                if playlist:
                    ...
                    logger.info(
                        f"Successfully fetched playlist '{playlist['name']}' (ID: {playlist_id})"
                    )
            except Exception as e:
    ...
    if modified_playlists:
        ...
        for playlist in modified_playlists:
            ...
            if "#auto" in playlist_name:
                logger.info(
                    f"Skipping auto playlist: '{playlist_name}' (ID: {playlist_id})"
                )
                db.log_action(
                    action_type="SKIP",
            ...
            if old_snapshot is None:
                ...
                db.insert_playlist_change(
                    playlist_id, playlist_name, "NEW", None, new_snapshot
                )
                db.log_action(
                ...
                db.insert_playlist_change(
                    playlist_id, playlist_name, "MODIFIED", old_snapshot, new_snapshot
                )
                db.log_action(
                    action_type="ADD_TO_QUEUE",
                    entity_type="PLAYLIST",
...
def update_songs():
    ...
    for playlist in modified_playlists:
        ...
        if not playlist_validation["valid"]:
            logger.error(
                f"Playlist '{playlist_name}' (ID: {playlist_id}) is no longer valid: {playlist_validation['error']}"
            )
            db.log_action(
            ...
            logger.info(
                f"Removed invalid playlist '{playlist_name}' from processing queue"
            )
            continue

        if not playlist_validation["accessible"]:
            logger.warning(
                f"Playlist '{playlist_name}' (ID: {playlist_id}) is no longer accessible: {playlist_validation['error']}"
            )
            db.log_action(
                action_type="REMOVE_FROM_QUEUE",
                entity_type="PLAYLIST",
            ...
            db.delete_queue(playlist_id)
            logger.info(
                f"Removed inaccessible playlist '{playlist_name}' from processing queue"
            )
            continue
        ...
        try:
            ...
            if new_tracks:
                logger.info(
                    f"Batch processing artist info for {len(new_tracks)} new tracks"
                )
                artist_data = spotify.process_tracks_with_batched_artists(
                    sp, new_tracks
                )
                ...
                for track in new_tracks:
                    ...
                    for artist in artists:
                        logger.info(
                            f"\tProcessing artist: {artist['name']} (ID: {artist['id']})"
                        )
                        artist_id = artist["id"]
                        ...
                        if artist_id in artist_data:
                            ...
                            logger.warning(
                                f"Artist {artist_id} not found in batch, fetching individually"
                            )
...
def cleanup_orphaned_data(currently_tracked_playlists):
    ...
    try:
        currently_tracked_ids = [
            playlist["id"] for playlist in currently_tracked_playlists
        ]
        ...
        try:
            ...
            if orphaned_playlists:
                logger.info(
                    f"\tFound {len(orphaned_playlists)} orphaned playlist(s) to remove"
                )
                ...
                for playlist in orphaned_playlists:
                    ...
                    try:
                        logger.info(
                            f"\t- Removing orphaned playlist: '{playlist_name}' (ID: {playlist_id})"
                        )
                        ...
                        cleanup_stats["orphaned_playlists"]["songs_removed"] += len(
                            playlist_songs
                        )
                        cleanup_stats["orphaned_playlists"]["changes_removed"] += len(
                            queue
                        )
                    except Exception as e:
                        logger.error(
                            f"\t- Failed to delete playlist '{playlist_name}': {e}"
                        )
            else:
        ...
        try:
            cleanup_stats["orphaned_playlist_songs"] = (
                db.delete_orphaned_playlist_songs()
            )
            if cleanup_stats["orphaned_playlist_songs"] > 0:
                logger.info(
                    f"\tRemoved {cleanup_stats['orphaned_playlist_songs']} orphaned playlist-song relationship(s)"
                )
            else:
                logger.info("\tNo orphaned playlist-song relationships found")
        except Exception as e:
            logger.error(
                f"\tError cleaning up orphaned playlist-song relationships: {e}"
            )
        ...
        try:
            ...
            if orphaned_songs:
                ...
                for song in orphaned_songs:
                    ...
                    try:
                        logger.info(
                            f"\t- Deleting orphaned song: '{song_name}' (ID: {song_id})"
                        )
        ...
        try:
            ...
            if cleanup_stats["orphaned_song_artists"] > 0:
                logger.info(
                    f"\tRemoved {cleanup_stats['orphaned_song_artists']} orphaned song-artist relationship(s)"
                )
            else:
        ...
        try:
            ...
            if orphaned_artists:
                logger.info(
                    f"\tFound {len(orphaned_artists)} orphaned artist(s) to delete"
                )
                ...
                for artist in orphaned_artists:
                    ...
                    try:
                        logger.info(
                            f"\t- Deleting orphaned artist: '{artist_name}' (ID: {artist_id})"
                        )
                        db.delete_artist(artist_id)
                        cleanup_stats["orphaned_artists"]["deleted"] += 1
                    except Exception as e:
                        logger.error(
                            f"\t- Failed to delete artist '{artist_name}': {e}"
                        )
            else:
        ...
        if playlist_stats["count"] > 0:
            logger.info(
                f"\tOrphaned playlists: {playlist_stats['deleted']}/{playlist_stats['count']} deleted"
            )
            logger.info(
                f"\t\tPlaylist-song relationships removed: {playlist_stats['songs_removed']}"
            )
            logger.info(
                f"\t\tChange records removed: {playlist_stats['changes_removed']}"
            )
        else:
            logger.info("\tNo orphaned playlists found")
        ...
        if cleanup_stats["orphaned_playlist_songs"] > 0:
            logger.info(
                f"\tOrphaned playlist-song relationships: {cleanup_stats['orphaned_playlist_songs']} cleaned"
            )
        else:
            logger.info("\tNo orphaned playlist-song relationships found")
        ...
        if song_stats["count"] > 0:
            logger.info(
                f"\tOrphaned songs: {song_stats['deleted']}/{song_stats['count']} deleted"
            )
        else:
            logger.info("\tNo orphaned songs found")
        ...
        if cleanup_stats["orphaned_song_artists"] > 0:
            logger.info(
                f"\tOrphaned song-artist relationships: {cleanup_stats['orphaned_song_artists']} cleaned"
            )
        else:
            logger.info("\tNo orphaned song-artist relationships found")
        ...
        if artist_stats["count"] > 0:
            logger.info(
                f"\tOrphaned artists: {artist_stats['deleted']}/{artist_stats['count']} deleted"
            )
        else:
            logger.info("\tNo orphaned artists found")

        total_cleaned = (
            playlist_stats["deleted"]
            + cleanup_stats["orphaned_playlist_songs"]
            + cleanup_stats["orphaned_song_artists"]
            + song_stats["deleted"]
            + artist_stats["deleted"]
        )
...
try:
    ...
    if "rate" in error_message and "limit" in error_message:
        ...
        logger.error(
            "The application will exit to prevent further rate limit violations."
        )
        logger.error(f"Error details: {str(e)}")
        exit(1)
    elif hasattr(e, "http_status") and e.http_status == 429:
        logger.error(
            "Spotify API returned HTTP 429 (Too Many Requests) during sync process."
        )
        logger.error(
            "The application will exit to prevent further rate limit violations."
        )
